      ...
      commands = {
          ...
          if #nodes == 0 then
            return -- No nodes selected, do nothing
					...
					local msg
					if #nodes > 1 then
						msg = "Are you sure you want to trash " .. #nodes .. " files?"
					elseif #nodes == 1 then
						local _, name = utils.split_path(nodes[1].path)
						msg = string.format("Are you sure you want to trash '%s'?", name)
					else
						return -- No nodes selected, do nothing
					end
					...
					inputs.confirm(msg, function(confirmed)
						...
						if result.code == 0 then
							vim.notify("Moved " .. #nodes .. " file(s) to trash.", vim.log.levels.INFO)
							require("neo-tree.sources.manager").refresh(state.name)
						else
							local error_message = "Failed to trash file(s).\n\n" .. (result.stderr or "")
